...
| [04-pipeline-architecture.md](./04-pipeline-architecture.md) | 6-stage pipeline design, what to keep/drop, pipeline diagram, C++ comparison table, sub-agent execution model (fresh agent per stage/pass invariant, 9 discrete invocations), all 6 stage specifications, per-stage I/O contracts, per-stage error codes and handling, preprocessor phase invariants, compiler phase characteristics |
...
| [06-execution-workflow.md](./06-execution-workflow.md) | Sub-agent orchestration model (spawn → process → write → terminate per stage/pass), 8 sub-agent invocations + validation, zero initial context invariant, sole information boundary, cleanup/lifecycle management, git configuration, disk space considerations, error diagnosis workflow with example |
