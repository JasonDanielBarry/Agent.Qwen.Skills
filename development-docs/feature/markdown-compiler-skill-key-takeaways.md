...
Source (.human.md)
   ↓
[1] Preprocessor
   - Macro expansion, file inclusion, conflict detection, verbose filler identification, contextual reasoning compression
...
⚠️ **Needs detailed stage specifications** — this architecture is a decision record but is insufficient for implementation. The following gaps must be filled before coding the compiler:
...
Modeled after C/C++ preprocessor (Stage 1 of the 6-stage pipeline above).
...
Open research items are tracked and answered in **`markdown-compiler-skill-questions.md`** under the **"Open Research Items"** section. Cross-reference:

| Topic | Question Ref | Status |
|-------|-------------|--------|
| Structured elements for AI agents | Q26, R1 | ❌ Undecided |
| Output format | Q7, R2 | ❌ Undecided |
| Validation methodology (functional equivalence test) | Q13–Q15, Q31, Q39, R3 | ⚡ Partially decided |
| Filler vs. context distinction rules | Q25, R4 | ⚡ Technique decided (preprocessing), rules TBD |

### Resolved: Pipeline Architecture (Q41)

The pipeline architecture is resolved. See **Q41** in `markdown-compiler-skill-questions.md` and the **"Compilation Pipeline — 6-Stage Design"** section above for the full analysis.

**Decision:** 6-stage streamlined pipeline — not a 1:1 C++ replication.
